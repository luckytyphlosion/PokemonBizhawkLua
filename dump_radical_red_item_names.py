...
def main():
    with open("radical_red_2.1.gba", "rb") as f:
        ...
        output = ""
        for i in range(750):
            f.seek(gItems + i * 0x2c)
            item_name_as_bytes = f.read(14)
            cur_item_name = charmap_convert_bytes_to_string(item_name_as_bytes)
            output += f"\"{cur_item_name}\", "
            if i != 0 and i % 7 == 0:
                output += "\n"

    with open("dump_radical_red_item_names_out.txt", "w+") as f:
        f.write(output)
