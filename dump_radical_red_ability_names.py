...
def main():
    with open("radical_red_2.1.gba", "rb") as f:
        gAbilityNames = 0x14b2870
        f.seek(gAbilityNames)
        output = ""
        for i in range(256):
            ability_name_as_bytes = f.read(17)
            cur_ability_name = charmap_convert_bytes_to_string(ability_name_as_bytes)
            output += f"\"{cur_ability_name}\", "
            if i != 0 and i % 7 == 0:
                output += "\n"

    with open("dump_radical_red_ability_names_out.txt", "w+") as f:
        f.write(output)
