...
def main():
    with open("radical_red_2.1.gba", "rb") as f:
        gMoveNames = 0x14bc258
        f.seek(gMoveNames)
        output = ""
        for i in range(896):
            move_name_as_bytes = f.read(13)
            cur_move_name = charmap_convert_bytes_to_string(move_name_as_bytes)
            output += f"\"{cur_move_name}\", "
            if i != 0 and i % 7 == 0:
                output += "\n"

    with open("dump_radical_red_move_names_out.txt", "w+") as f:
        f.write(output)
